...
average change in job access per demographic group by adjustment
...
sf_delta_axs <- origins_i_sf %>%
  mutate(id = as.numeric(id)) %>%
  inner_join(mbta_axs_changes %>% filter(change_name == 'Spring 2023'), by='id') %>%
  filter(delta_p50 != 0)

tm_shape(sf_delta_axs) +
    tm_polygons(fill = 'delta_p50', 
            fill.scale = tm_scale_continuous(values='brewer.rd_yl_gn'),
            fill.legend = tm_legend("Change in Jobs Accessible"),
            col_alpha = 0) #+
