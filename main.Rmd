...
gtfs_metrics %>%
  ...
  ggplot(aes(fill=route_category, y=total_runtime_minutes, x=service_dt)) + 
    ...
    ggtitle("Total Scheduled MBTA Runtime by Route Category (Tuesdays, 2019-present)")
```
...
## Calculate Origin-Destination Travel Time Matrix
...
for (i in 1:length(mbta_tues_lookup)) {
  feed_day <- names(mbta_tues_lookup)[[i]]
  feed_curr <- mbta_tues_lookup[[i]]
...
ttm_from_day <- function(feed_day) {
...
axs_lookup <- names(mbta_tues_lookup) %>%
  lapply(ttm_from_day) %>% 
  lapply(axs_window_percentiles)
```
