...
# Data Pulling and Pre-Processing
...
To decide which dates we want to pull to represent each service change, we first calculate scheduled quantity of service (SQoS) metrics at the route-day level from the MBTA's compressed GTFS archive, mainly so we can identify and exclude dates whose schedules have a lot of temporary shuttling (Rail Replacement Bus service, in MBTA terminology). However, the SQoS metrics will be useful context for the main analysis too.
...
Wrapper function for calculating the daily SQoS metrics for any date range, even ones that span multiple years. Iterates over years because the MBTA compressed GTFS archive tables are partitioned into separate years.
...
Visualize the SQoS "inputs" over time which represent the context / overall service levels & trends for the period of service changes being analyzed as a case study

```{r}
gtfs_metrics <- read.csv(path(data_path, "mbta_gtfs_metrics_route.csv"))
```

Identify service days with suspiciously low total runtimes (likely data anomalies) to filter out in the subsequent charts.
...
#ggsave(path(outputs_path,'sched_mbta_runtime.svg'), sched_plot, width=11, height=6, units='in')
sched_plot
```

Versions of this chart over shorter date ranges were also used to identify dates before and after the MBTA service changes of interest that had minimal (or roughly equivalent) Rail Replacement Bus service. This informed the exact dates that are hard-coded into the next step.
...
gtfs_metrics %>%
  mutate(service_dt = as.POSIXct(strptime(as.character(service_day), '%Y%m%d')),
         route_id = case_match(route_id, '441442' ~ '441/442', .default=route_id)) %>%
  filter(route_id %in% c('34', '441/442', '4'), weekdays(service_dt) == 'Tuesday', 
  ...
  annotate("segment", x=as.POSIXct('2024-08-25'), y=1100, yend=900, color='red',
           arrow=arrow(type = "closed", length = unit(0.015, "npc"))) +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x='Service Date', y='Daily Minutes of Vehicle Runtime', linetype='Bus Route',
...
The other piece is to create a table summarizing these changes in SQoS for the report. I will tack on destination access rows to these SQoS rows in the "Summary stats per change" section.
...
Functions for calculating destination access by summing up the opportunities (jobs, in this case) within the destinations that are reachable from each origin.
...
Read the OD travel time matrices for a set of dates, calculate accessibility per percentile, and save the results. This cell took ~2min per date for MBTA blocks (n=~80k).  
...
Get race/ethnicity data per origin, clean it, and join it in.
...
Create dataframes of the before-and-after of destination access for each service change
...
mbta_axs_changes %>%
  ...
  scale_fill_manual(values=c('#27b376', '#bf212f', '#264b96')) +
...
Befores, afters, and deltas of average job access for each race/ethn group and each departure timing percentile per service change
...
combine the SQoS summary with the access summary and save to csv
...
Future idea: quadrant or other viz of chg axs vs chg in tod ptile swing of axs ("consistency"?)

### Demographic breakdown visualizations

average change in job access per demographic group by svc change. can graph before or after instead to get differences in average existing access

```{r}
avg_changes_raceethn <- mbta_axs_changes %>%
  ...
  summarise(
    mean_axs_b4 = wtd.mean(before_p50, weights=pop),
...
Calculate ratios of percent changes in access for protected vs nonprotected populations and compare with MBTA thresholds for defining disparate impacts

```{r}
pct_delta_comparison <- avg_changes_raceethn %>%
  pivot_wider(id_cols='change_name', names_from='race', values_from=c('mean_axs_b4', 'mean_axs')) %>%
  mutate(
    pct_delta_protected = `mean_axs_Non-white (or Hispanic of any race)` / 
      `mean_axs_b4_Non-white (or Hispanic of any race)`,
    pct_delta_nonprotected = `mean_axs_White (non-Hispanic)` / `mean_axs_b4_White (non-Hispanic)`,
    ratio_pct_delta = pct_delta_protected / pct_delta_nonprotected,
    disparate = case_when(
      pct_delta_protected < 0 & ratio_pct_delta > 1.2 ~ T,
      pct_delta_protected > 0 & ratio_pct_delta < 0.8 ~ T,
      .default = F
    ),
    .keep='unused'
  )
...
```{r}
pct_delta_comparison %>%
  write.csv(path(outputs_path, "pct_delta_comparison.csv"), row.names=F)
```

Chart of average absolute changes in job access per population per service change

``` {r}
avg_changes_raceethn_plot <- avg_changes_raceethn %>%
  ggplot(aes(fill=race, x=race, y=mean_axs)) + 
  geom_bar(position='dodge', stat='identity') +
  scale_y_continuous(labels=comma) +
  scale_fill_manual(values=c("#156082", '#FFC000')) +
...
avg_changes_raceethn_plot
```

```{r}
ggsave(path(outputs_path,'avg_changes_raceethn.svg'), avg_changes_raceethn_plot, width = 11, height=6)
```
...
Prepare spatial data with the changes in median-percentile job access per origin

```{r}
sf_delta_axs <- origins_i_sf %>%
  mutate(id = as.numeric(id)) %>%
  inner_join(mbta_axs_changes, by='id') %>%
  # # uncomment to filter out some of the service changes to reduce the number of facets
  # filter(!(change_name %in% c('Winter 2023', 'Spring 2024'))) %>%
  mutate(change_name=factor(change_name, levels=names(mbta_changes))) %>%
  group_by(id) %>%
...
Facet map of changes in access per origin per service change:

``` {r, fig.width=6, fig.height=6}
chgAxs_facetMap <- sf_delta_axs %>%
  filter(!(change_name %in% c('Winter 2023', 'Spring 2024'))) %>%
  mutate(change_name = case_match(
    change_name, 'Spring and Summer 2023' ~ 'Spring & Summer 2023', 
    'Winter 2025 (BNR Phase 1)' ~ 'Winter 2025 (BNR Ph. 1)', .default=change_name
    ), 
    change_name = factor(
      change_name, levels=c('GLX Medford Opening', 'Spring & Summer 2023', 'Fall 2023', 
                            'Summer 2024', 'Fall 2024', 'Winter 2025 (BNR Ph. 1)')
      )
  ) %>%
  tm_shape() +
  tm_polygons(fill = 'delta_p50',  
          fill.scale = tm_scale_continuous_pseudo_log(
            ...
            ), # brewer.rd_yl_gn for red/yellow/green, brewer.prgn for purple/green
          fill.legend = tm_legend("Change in Job Access", reverse=TRUE, item.height=5),
          col_alpha = 0) +
  tm_facets("change_name", scale.factor=3, nrow=3) +
...
```{r}
tmap_save(chgAxs_facetMap, path(outputs_path,'chg_access_facetmap.jpg'), width=6, height=6, units='in')
```

Future idea: map of change in 15-to-85 range
Future idea: spatial autocorrelation or other statistics

### Summary stats across dates

population-weighted average job access per service change, per departure timing percentile, and per race/ethn group
...
Future idea: calculate gini & palma indices
...
Plot line of avg job access with departure timing variability around it
